...
par(mar=c(3.5,3.5,.5,3.6))
require(fields)
require(xtable)
#setwd("~/gaming/trabajos/conquerClub/SociosPorConveniencia/Imagenes")

ftot_perf <- read.csv("../datos/social_learning.csv", header =T)
...
faithfulness[faithfulnessLen <=6] <- NA
...
mtext("Loyalty", side=1, line=1.75,cex = 2)
mtext("Team-oriented behaviour", side=2, line=1.75,cex = 2)
box()
